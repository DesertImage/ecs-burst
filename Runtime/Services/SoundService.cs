...
{
    public class ServiceSound : IService, IAwake, ISoundLayerManager
    {
        ...
        {
            return !_soundLayers.TryGetValue(layerId, out var layer) ? default : layer.Play(ids, volume, pitch);
        }
            ...
            bool forceStop = false, bool isSingle = false)
        {
            ...
            if (_soundLayers.TryGetValue(layerId, out var layer))
            {
            ...
            if (forceStop)
            {
                foreach (var obj in _playingSounds.Values)
                {
                    obj[0]?.OnCreate();
            ...
            if (_playingSounds.TryGetValue(soundId, out var instances))
            {
            ...
            if (_playingSounds.TryGetValue(soundId, out instances))
            {
        ...
        {
            _soundLayers.TryGetValue(id, out var layer);
        ...
        public void ReturnInstance(SoundBase soundBase)
        {
            ...
            if (_playingSounds.TryGetValue(idHash, out var instances))
            {
